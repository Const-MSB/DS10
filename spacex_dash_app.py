...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df, values='class', names='Launch Site', title='Launch Site Result')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.pie(filtered_df, names='class', title=f'Launch Site Result: {entered_site}')
    return fig
...
def get_scatter_chart(selected_site, payload_range):
    minV, maxV = payload_range
    if selected_site == 'ALL':
        df = spacex_df
    else:
        df = spacex_df[spacex_df['Launch Site'] == selected_site]
    df_mask = df[(df['Payload Mass (kg)'] > minV) & (df['Payload Mass (kg)'] < maxV)]
